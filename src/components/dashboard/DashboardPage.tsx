...
import { dashboardApi, interviewsApi } from "../../api/client";
...
export function DashboardPage() {
  const { status: appStatus, data: dashboard } = useAsync(
    () => dashboardApi.applications(), []
  );
  const { data: interviews } = useAsync(() => interviewsApi.list(), []);

  const applications = dashboard?.items ?? [];
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="p-5 lg:col-span-2">
          ...
          {recentApps.length === 0 ? (
            ...
            <div className="space-y-2">
              {recentApps.map((app) => (
                <div key={app.application_id} className="flex items-center gap-4 text-sm py-1">
